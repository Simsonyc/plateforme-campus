  ...
  return (
    <main style={{ padding: '2rem', fontFamily: 'system-ui', background: '#f8fafc', minHeight: '100vh' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: '2rem', fontWeight: 700, color: '#1e293b', margin: '0.5rem 0 0.5rem' }}>👥 Membres</h1>
        <p style={{ color: '#64748b', marginBottom: '2rem' }}>{result.length} adhésion(s)</p>
            ...
            <div key={member.id} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
              <div>
                ...
                <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '0.25rem' }}>
                  Rôle : {member.membershipRole} — Depuis le {new Date(member.createdAt).toLocaleDateString('fr-FR')}
                </div>
              </div>
              <span style={{ background: statusColors[member.status]?.bg || '#f1f5f9', color: statusColors[member.status]?.color || '#64748b', fontSize: '0.75rem', fontWeight: 500, padding: '0.25rem 0.75rem', borderRadius: '999px', marginLeft: '1rem' }}>
                {member.status}
              </span>
            </div>
          ))}
